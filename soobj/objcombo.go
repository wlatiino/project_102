...
func CrtObjCmb(o ObjCmb) map[string]interface{} {
	...
	if o.Sqlstm == "" {
		if o.Table == "" {
			SO_Class.Log.Println(false, "combo", "object Struct 'table' cannot empty")
			return nil
		}
		...
		sqlstm =
			SO_Class.Fmt.Sprint(
				`
				Select 
				rtrim(tssycd) as "kode", rtrim(tssynm) as "nama"
				From tblsys 
	...
	if err != nil {
		SO_Class.Log.Println(true, "combo record tidak ada!!! (no rows effected) ")
		SO_Class.Log.Println(true, "sqlstm : ", sqlstm)
		SO_Class.Log.Println(true, "err : ", err)
		return nil
	} else {
		if data == nil {
			SO_Class.Log.Println(false, "combo record tidak ada!!! (no rows effected) ")
...
func init() {
	SO_Class.Log.Println(true, "Masuk soobj-objCmb-init()")
}

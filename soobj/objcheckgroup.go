...
func CrtObjChg(o ObjChg) map[string]interface{} {
	...
	if o.Sqlstm == "" {
		if o.Table == "" {
			SO_Class.Log.Println(false, "check group", "object Struct 'table' cannot empty")
			return nil
		}
	...
	if err != nil {
		SO_Class.Log.Println(true, "check group record tidak ada!!! (no rows effected) ")
		SO_Class.Log.Println(true, "sqlstm : ", sqlstm)
		SO_Class.Log.Println(true, "err : ", err)
		return nil
	} else {
		if data == nil {
			SO_Class.Log.Println(false, "check group record tidak ada!!! (no rows effected) ")
...
func init() {
	SO_Class.Log.Println(true, "Masuk soobj-objChg-init()")
}
